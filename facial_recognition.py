...
parser.add_argument('-i', '--input', type=str, required=True,
                    help='directory of input known faces')
...
def transform(image:np.ndarray, face_locations):
    coord_faces = []
    for face in face_locations:
        rect = face.top(), face.right(), face.bottom(), face.left()
        coord_face = max(rect[0], 0), min(rect[1], image.shape[1]), min(
            rect[2], image.shape[0]), max(rect[3], 0)
        coord_faces.append(coord_face)
    return coord_faces


def encode_face(image:np.ndarray)->tuple:
...
def easy_face_reco(frame, known_face_encodings, known_face_names):
    ...
    # ENCODING FACE
    face_encodings_list, face_locations_list, landmarks_list = encode_face(
        rgb_small_frame)
    face_names = []
    ...
    for (top, right, bottom, left), name in zip(face_locations_list, face_names):
        ...
        cv2.putText(frame, name, (left + 2, bottom - 2),
...
if __name__ == '__main__':
    ...
    known_face_names = [os.path.splitext(ntpath.basename(file_))[
        0] for file_ in files]
